...
with tab2:
    ...
    with col1:
        w0 = st.slider("Peso W₀", min_value=0.00, max_value=10.0, value=5.0, step=0.01)
        x0 = st.number_input("Entrada X₀", value=0.0)
    with col2:
        w1 = st.slider("Peso W₁", min_value=0.00, max_value=10.0, value=5.0, step=0.01)
        x1 = st.number_input("Entrada X₁", value=0.0)
...
with tab3:
    ...
    with col1:
        w0 = st.slider("Peso W₀ ", min_value=0.00, max_value=10.0, value=1.0, step=0.01)
        x0 = st.number_input("X₀ ", value=0.0)
    with col2:
        w1 = st.slider("Peso W₁ ", min_value=0.00, max_value=10.0, value=2.0, step=0.01)
        x1 = st.number_input("Entrada X₁ ", value=0.0)
    with col3:
        w2 = st.slider("Peso W₂ ", min_value=0.00, max_value=10.0, value=3.0, step=0.01)
        x2 = st.number_input("Entrada X₂ ", value=0.0)
